...
**The turn is visible, and it is the only thing Send is gated on.** The presence frame carries two separate facts: `waiting` — an agent is parked on `wait` right now — and `turn`, the review's own record of whose move it is. The header states a short fixed label off them, and the turn wins: "Agent is working" (whether the agent is reading or already implementing), or "Waiting for your feedback", or "No agent is waiting". The full sentence is the label's tooltip — "the agent has your feedback", or the plan `work` declared ("implementing: splitting the helper out"), or "an agent is waiting for your feedback", or "no agent is waiting — send anyway, the feedback is queued" — because a plan in the header's corner ran long, got cut off, and repeated the conversation below. The tooltip is a hover-only convenience — keyboard and touch never reach a `title` — and not where the status is kept: the conversation panel is the canonical place for the agent's status. The wording lives once, in `browser/turn-words.ts`, because the foot of the conversation says it in full: a line with three breathing dots in the place the answer will be written, which is where the eye goes after Send. The served page states the turn as well — it is on the record, so a reload mid-silence is right in the first paint rather than one SSE frame later. A reviewer who asked for less motion keeps the line, its dots up and still: the line is the news, and the movement was only what made it easy to catch.
