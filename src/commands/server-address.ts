...
/**
 * `/health` is the whole test; the request is bounded because a non-HTTP
 * process holding the port would wait forever. `undefined` means "not our
 * server", never "our server, unknown version": the two lead to different
 * commands.
 */
...
/**
 * False means nothing was listening — the desired end state either way. Lives
 * here rather than in `stop` because a stale server is replaced by the same
 * request `stop` makes.
 */
