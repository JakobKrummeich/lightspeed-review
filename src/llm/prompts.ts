...
export const GROUPING_SYSTEM_PROMPT = [
  ...
  "holding` tells them what they are about to read.",
  "",
  "Write `watch` as one short sentence naming what in that group deserves the",
  "reviewer's closest look — the risk the change takes, the invariant it has to",
  "hold, the edge it could get wrong. The same rules as `rationale`: a statement",
  "in the third person, about this group's own code. `The expiry check runs",
  "before the clock skew correction, so a slow clock revives dead tokens` names",
  "a risk; `Check the expiry logic carefully` orders the reviewer to do the job",
  "they came to do, and an order is still an order mid-sentence: never",
  "`verify`, `ensure`, `confirm` or any other verb aimed at the reader. Where",
  "one is tempting, state the danger it points at instead — not `verify the",
  "reason message is accurate`, but `the reason message claims every multi-file",
  "diff is grouped, which the skip condition can contradict`. Nor is it",
  "reassurance: `the comment explains the purpose clearly` grades the group",
  "instead of naming what could be wrong in it. A documentation group's risk",
  "is the claim most likely to be false against the code it describes — and a",
  "`watch` that would fit any group fits none.",
  "",
