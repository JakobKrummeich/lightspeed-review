...
/** Pure, like `diff-view.ts`: markup asserted without a DOM. */

/**
 * `branch` (vs the target branch) is the default: a file that changed under the
 * reviewer still has to be reviewed as a whole.
 */
...
/**
 * A single grep-able name, not a repeated literal. Allowlist on purpose: new
 * forms are unannotatable until someone says otherwise.
 */
export const ANNOTATABLE_FORM: FileForm = "branch";

/**
 * The store fetches, caches and repaints these, keyed by this type so a new
 * `FileForm` must declare which side it stands on.
 */
export type FetchedForm = Exclude<FileForm, "branch">;

/** `full` is out: it shows one whole version and has no diff states to speak for. */
...
/**
 * Which pair a file carries is `diff-view.ts`'s call; options name what they
 * show, not what pressing does.
 */
...
/**
 * Git is a subprocess, not instant. Worded per view: "since you approved"
 * claims a tick the other switch never got.
 */
...
/**
 * Literal-keyed records so a state added server-side stops compiling here until
 * both views answer it. Voices differ: each sentence may only claim what
 * actually happened (a tick given vs a round read).
 */
...
/**
 * All historical paths named and renames followed: `git diff` given only
 * today's path prints nothing for a renamed file — the case where this message
 * matters most.
 */
