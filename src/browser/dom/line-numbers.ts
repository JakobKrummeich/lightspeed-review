...
/**
 * Side by side each column carries its own gutter, so which column the line
 * sits in decides the version; unified, both numbers share one cell and the
 * empty one marks a line that belongs to the other version only. The single
 * place that knows the gutter markup: line anchors and whole-file highlighting
 * both hang off these numbers.
 */
...
/** Empty when the file is rendered unified. */
...
/**
 * The inverse of `lineNumbers`, walked over the same elements a selection
 * walks, so an anchor read off a selection is found by the very numbers it was
 * written from.
 */
